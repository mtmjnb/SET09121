...
const sf::Vector2f box2d::bv2_to_sv2(const b2Vec2& in) {
    return sf::Vector2f(in.x * Parameters::physics_scale, (in.y * Parameters::physics_scale));
}

// Convert from Vector2f to a b2Vec2
const b2Vec2 box2d::sv2_to_bv2(const sf::Vector2f& in) {
    return { in.x * Parameters::physics_scale_inv, in.y * Parameters::physics_scale_inv };
}
...
const sf::Vector2f box2d::invert_height(const sf::Vector2f& in) {
    return sf::Vector2f(in.x, Parameters::game_height - in.y);
}
...
b2BodyId box2d::create_physics_box(b2WorldId& world_id, const bool dynamic, const std::shared_ptr<sf::RectangleShape>& rectangle_shape) {
    return create_physics_box(world_id, dynamic, rectangle_shape->getPosition(), rectangle_shape->getSize());
}